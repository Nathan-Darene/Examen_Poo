...
class Grille:
    def __init__(self, decalage_x, decalage_y):
        ...
        for key, image in self.images_navires.items():
            if image is None:
                print(f"Erreur de chargement de l'image pour {key}")
        self.taille_navires = {
            "porte_avions": 5,
            "croiseur": 4,
            "contre_torpilleur": 3,
    ...
    def dessiner(self, screen):
        ...
        for navire in self.navires:
            ...
            if orientation == 'H':
                ...
                image_navire = pygame.transform.rotate(image_navire, 90)  # Rotation de 90 degrés pour le mode vertical
    ...
    def changer_orientation_navire(self, index):
        ...
        if nouvelle_orientation == 'H' and colonne + taille <= TAILLE_GRILLE:
            for i in range(taille):
                self.grille[ligne][colonne + i] = 1
            self.navires[index] = (ligne, colonne, type_navire, nouvelle_orientation)
        elif nouvelle_orientation == 'V' and ligne + taille <= TAILLE_GRILLE:
            for i in range(taille):
...
class Positionnement:
    ...
    def dessiner(self, screen):
        pygame.draw.rect(screen, BLEU, self.rect)
        font = pygame.font.SysFont("sans-serif", 20)
        text = font.render("Changer Orientation", True, BLANC)
...
class JeuBatailleNavale:
    def __init__(self):
        ...
        self.positionnement = Positionnement(100, 800, 200, 50)  # Bouton pour changer l'orientation
        ...
        while en_cours:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.positionnement.est_clique(pos) and self.selectionne is not None:
                        self.grille1.changer_orientation_navire(self.selectionne)
                    else:
            ...
            self.positionnement.dessiner(self.ecran)
            pygame.display.flip()
...
def configurer_jeu():
    ...
    jeu.grille1.placer_bateau(6, 1, "contre_torpilleur", 'V')
    jeu.grille1.placer_bateau(8, 1, "sous_marin", 'V')
    jeu.grille1.placer_bateau(9, 1, "torpilleur", 'V')
