...
class Grille:
    ...
    def dessiner(self, screen):
        ...
        # Dessiner les images des navires pour la première grille
        for navire in self.navires:
            ...
            if orientation == 'H':
                ...
                image_navire = pygame.transform.rotate(image_navire, 90) # Rotate by 90 degrees for vertical orientation
                image_navire = pygame.transform.scale(image_navire, (TAILLE_CELLULE, TAILLE_CELLULE * taille + MARGE * (taille - 1)))
                screen.blit(image_navire, ((MARGE + TAILLE_CELLULE) * colonne + MARGE + self.decalage_x,
                                (MARGE + TAILLE_CELLULE) * ligne + MARGE + self.decalage_y))

        # Dessiner les images des navires pour la deuxième grille
        for navire in self.navires:
            ligne, colonne, type_navire, orientation = navire
            image_navire = self.images_navires[type_navire]
            taille = self.taille_navires[type_navire]
            ...
            if orientation == 'H':
                image_navire = pygame.transform.scale(image_navire, (TAILLE_CELLULE * taille + MARGE * (taille - 1), TAILLE_CELLULE))
                screen.blit(image_navire, ((MARGE + TAILLE_CELLULE) * colonne + MARGE + self.decalage_x + self.decalage_x,
                                (MARGE + TAILLE_CELLULE) * ligne + MARGE + self.decalage_y))
            elif orientation == 'V':
                image_navire = pygame.transform.rotate(image_navire, 90) #90 deg
                image_navire = pygame.transform.scale(image_navire, (TAILLE_CELLULE, TAILLE_CELLULE * taille + MARGE * (taille - 1)))
                screen.blit(image_navire, ((MARGE + TAILLE_CELLULE) * colonne + MARGE + self.decalage_x + self.decalage_x,
                                (MARGE + TAILLE_CELLULE) * ligne + MARGE + self.decalage_y))
...
def configurer_jeu():
    ...
    # Placez des navires sur la deuxième grille pour les tests
    jeu.grille2.placer_bateau(0, 0, "porte_avions", 'V')
    jeu.grille2.placer_bateau(4, 1, "croiseur", 'V')
    jeu.grille2.placer_bateau(6, 1, "contre_torpilleur", 'V')
    jeu.grille2.placer_bateau(8, 1, "sous_marin", 'V')
    jeu.grille2.placer_bateau(9, 1, "torpilleur", 'V')
